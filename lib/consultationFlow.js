...
const { BRAND_VOICE, CONSULTATION_PLAN_NOTES } = require('./knowledgeBase');
 ...
 */
async function maybeHandleConsultationPayment({ enabled, type, memoryId, message, intent }) {
  ...
  if (CANCEL_WORDS.test(message)) {
    ...
    return {
      reply: 'Bilkul, koi baat nahi! Jab bhi aap consultation plan ke liye ready hon, bata dijiyega. - Mannat',
      leadStatus: 'WARM', productInterest: state.planLabel || 'Consultation', escalate: false, escalateReason: '',
    };
  }

  try {
    if (state.stage === 'collecting_contact') return await handleCollectingContact(memoryId, state, message);
    if (state.stage === 'awaiting_receipt') return await handleAwaitingReceipt(memoryId, state, message);
  } catch (err) {
    console.error('[consultationFlow] unexpected error, escalating with what we have:', err.message);
    await clearSession(memoryId);
    return {
      reply: 'Isme thodi technical dikkat aa rahi hai - koi baat nahi, main is conversation ko humari team se connect kar rahi hoon taaki wo aapki payment/consultation personally sort kar sakein. - Mannat',
...
async function handleCollectingContact(memoryId, state, message) {
  ...
  if (state.slots.name && state.slots.phone) {
    ...
    return {
      reply: reply || `Thank you! Aapka ${state.planLabel} ka payment link yeh raha: ${plan.url}\nPayment ke baad receipt/screenshot yahin share kar dijiyega. - Mannat`,
  ...
  return {
    reply: reply || `Bilkul! Payment link bhejne se pehle please apna naam aur contact number share kar dijiye, taaki team follow up kar sake. - Mannat`,
    leadStatus: 'HOT', productInterest: state.planLabel, escalate: false, escalateReason: '',
  };
}

async function handleAwaitingReceipt(memoryId, state, message) {
  const plan = getConsultationPaymentLink(state.planKey);
  ...
  return {
    reply: reply || `Thank you! Humari team payment confirm karke aapko jald hi contact karegi. - Mannat`,
    leadStatus: 'HOT', productInterest: state.planLabel, escalate: true, escalateReason: caseFile,
  };
}

module.exports = { maybeHandleConsultationPayment };
