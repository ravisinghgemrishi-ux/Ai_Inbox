...
const { BRAND_VOICE, CONSULTATION_PLAN_NOTES, detectReplyLanguageNote } = require('./knowledgeBase');
...
async function maybeHandleConsultationPayment({ enabled, type, memoryId, message, intent, existingMemory = '' }) {
  ...
  try {
    if (state.stage === 'collecting_contact') return await handleCollectingContact(memoryId, state, message, languageNote);
    if (state.stage === 'awaiting_receipt') return await handleAwaitingReceipt(memoryId, state, message, languageNote);
  } catch (err) {
...
async function handleCollectingContact(memoryId, state, message, languageNote = '') {
...
async function handleAwaitingReceipt(memoryId, state, message, languageNote = '') {
  const plan = getConsultationPaymentLink(state.planKey);
