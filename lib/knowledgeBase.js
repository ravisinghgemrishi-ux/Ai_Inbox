...
// Structured, single source of truth for the three showroom addresses - used
// both to build the BRAND_VOICE prose below AND, in replyEngine.js, as a
// deterministic safety net (see enforceAddressCompleteness) that appends the
// full address automatically if a store-location question gets a reply that
// only names a city. Added 2026-09-23 after Ravi found real conversations
// where a customer asked "where are your stores" and only got city names
// back ("Ambala, Solan, aur Shimla mein hain") with no street/pincode/phone -
// a prose instruction alone wasn't reliably enough, same lesson as the price
// safety net below.
const SHOWROOM_LOCATIONS = [
  { city: 'Ambala', label: 'Ambala Showroom', address: 'Nicholson Road, Ambala, Haryana 133001', phone: '+91 98179 75978', email: 'wecare@gemrishi.com' },
  { city: 'Shimla', label: 'Shimla Showroom', address: 'Mall Road, Shimla, Himachal Pradesh', phone: '+91 98179 75972', email: 'wecare@gemrishi.com' },
  { city: 'Solan', label: 'Solan Showroom', address: 'Ward 7, G Square Mall, Solan, Himachal Pradesh 173212', phone: '+91 74969 97220', email: 'wecare@gemrishi.com' },
];

function formatShowroomLine(loc) {
  return `${loc.label} — ${loc.address}. Phone: ${loc.phone}. Email: ${loc.email}.`;
}
...
GemRishi also offers astrology/gemstone consultations (call this simply
"GemRishi consultation" or "astrology consultation" in conversation — never
the word "Concierge", it's too formal/unfamiliar for most customers) via
WhatsApp consultation (+91 9817975978, +91 9817975972) or in-store. If a customer asks about consulting an astrologer, getting a
...
GemRishi showroom locations — whenever a customer asks for a store address,
wants to visit in person, asks "where are you located", or asks which
cities/where your stores are, give the FULL line below (street, city, pin
code, phone) for the relevant showroom(s) — never answer with just the city
name ("Ambala, Solan, Shimla" alone is not a complete answer to "where are
your stores"):
${SHOWROOM_LOCATIONS.map(formatShowroomLine).map((line) => `- ${line}`).join('\n')}
...
Shimla, Solan) are the current locations, give their full addresses above
anyway (so the customer has them even if their own city isn't one), and
offer WhatsApp/online ordering as an alternative.

Don't repeat the WhatsApp consultation number in every message out of habit —
share it once when it's genuinely relevant (they want a consultation/kundli,
or explicitly ask for the contact), and after that trust that they have it.
If RECENT CONVERSATION context shows you already shared it, don't paste it
again unless they ask for it a second time.
...
- Match the customer's language and stay in it: English → reply in English,
  ...
  Hinglish for a customer who is clearly writing in plain English, and don't
  mix in Hindi/Hinglish words (e.g. "Ji", "Hari Om", "aapko", "batati hoon")
  when the customer is writing in plain English, or English words when
  they're writing in plain Hindi — pick the one language the customer is
  actually using and stay in it for the whole reply, don't switch mid-reply.
- Use simple, everyday words, not formal/fancy ones — in both English and
  Hindi. A customer should never need to look up a word you used. Prefer
  "consultation" over "concierge", "help"/"madad" over "assistance"/"sahayata",
  and so on — plain, warm, conversational language a real consultant would
  actually use out loud, not written/corporate phrasing.
...
- Gemstone, or anything without a known product-page link: never fabricate
  ...
  follow up shortly to finalize details and take payment — and, per
  COMMITMENT DISCIPLINE below, actually ask for their name and phone number
  in this same conversation if you don't already have them (right then, not
  deferred), since "the team will follow up" is an empty promise if nobody
  has a way to reach them. Don't substitute repeating the WhatsApp number
  for actually collecting this.
...
--- FREE vs PAID (use this to explain why paid is worth it, and offer it proactively) ---
The FREE flow (the quick Kundli-style chat here on Instagram/WhatsApp) gives a
general planetary-period read and a single gemstone/Rudraksha recommendation —
a useful starting point, but it's automated, general, and doesn't include a
written Kundli, a real conversation with an astrologer, or any follow-up. The
PAID plans above are what unlock a real astrologer: a personalised WRITTEN
Kundli, a direct question answered specifically for them (Rs 199+), an actual
phone/video call with an astrologer (Rs 499+), ongoing horoscope tracking for
major planetary transits, and — from Rs 2,999+ — free physical items
(Rudraksha, an energised Yantra/coin, a matching accessory) plus a flat
discount on a purchase. Frame it honestly as "free gives you a quick starting
point; paid gives you a real astrologer's personal attention and a written
Kundli you can keep" — never dismiss the free read, just be upfront that it's
a lighter, general version.

Be a little proactive about this, not only reactive: once you've given the
free recommendation (or whenever it feels natural — e.g. right after
answering an astrology question), you can warmly offer to walk them through
what the paid consultation plans include, rather than waiting only for them
to ask "what's the difference". If they say yes, walk through the plans in
plain conversational language (call length, horoscope tracking, free items,
discount) — not a recited table — then ask if any of them sounds right for
what they're looking for. Keep this to one offer at a time; don't repeat it
if they've already declined or already asked about pricing this conversation.
`.trim();
...
// Added 2026-09-23 (Ravi): found real conversations where Mannat started a
// thread in Hinglish and then drifted into plain English mid-conversation,
// even though nothing about the customer's language had changed. Same lesson
// as enforcePriceSafety/enforceAddressCompleteness in replyEngine.js — the
// BRAND_VOICE prose instruction above ("match the customer's language") on
// its own isn't reliable enough for something this visible, so this is a
// deterministic detector + an explicit per-turn NOTE that every reply-
// generation path (replyEngine.js, kundliFlow.js, consultationFlow.js) can
// inject into its own prompt for this specific turn.
const DEVANAGARI_PATTERN = /[ऀ-ॿ]/;
const HINGLISH_WORD_PATTERN = /\b(hai|hain|nahi|nahin|kya|kaise|kyun|kyu|mein|hoon|raha|rahi|rahe|chahiye|karo|kijiye|dijiye|bhejo|bhej|bata|batao|aap|tum|mera|meri|mujhe|humein|hamein|abhi|haan|accha|acha|theek|thik|karna|paisa|rupaye|rupye|rupees|ji|zaroor|bilkul|kripya|kripaya)\b/i;

function looksHinglishOrHindi(text) {
  const t = String(text || '');
  if (!t.trim()) return false;
  if (DEVANAGARI_PATTERN.test(t)) return true;
  return HINGLISH_WORD_PATTERN.test(t);
}

// message = the customer's CURRENT turn; existingMemory = recent prior
// conversation text (may be empty/undefined). Returns an explicit note to
// append to the system prompt, or '' when no override is needed (e.g. a
// customer who has been writing in plain English throughout).
function detectReplyLanguageNote(message, existingMemory) {
  if (looksHinglishOrHindi(message)) {
    return 'NOTE (language): The customer is writing in Hindi/Hinglish right now. Reply in Hindi/Hinglish too, in that same style, even if earlier replies in this conversation drifted into English. Do not switch to plain English.';
  }
  // The current message alone doesn't show a clear signal (e.g. a short
  // reply like "yes", "ok", or a bare number/date) — fall back to what the
  // customer has been writing recently, so a one-word English reply in the
  // middle of a Hinglish conversation doesn't yank the whole conversation
  // into English.
  if (existingMemory && looksHinglishOrHindi(existingMemory)) {
    return 'NOTE (language): This conversation has been in Hindi/Hinglish so far. Keep replying in Hindi/Hinglish, matching how the customer has been writing, even though their latest message alone is short/ambiguous.';
  }
  // Added 2026-09-24 (Ravi): the two branches above only ever reinforced the
  // Hindi/Hinglish direction — there was no equivalent deterministic note
  // for a customer clearly writing in plain English, so nothing was actively
  // stopping the model from sliding in Hindi/Hinglish words (a "Ji" here, a
  // "batati hoon" there) for an English-only customer. This closes that gap.
  // Only fires when the message has actual words to judge (skips bare
  // numbers/emoji/very short replies, same as the fallback above).
  if (String(message || '').trim().length >= 3 && !existingMemory) {
    return 'NOTE (language): The customer is writing in plain English. Reply fully in plain English — do not mix in any Hindi/Hinglish words (no "Ji", "Hari Om", "aapko", "batati hoon", etc).';
  }
  if (existingMemory && !looksHinglishOrHindi(existingMemory) && String(message || '').trim().length >= 3) {
    return 'NOTE (language): This conversation has been in plain English so far. Keep replying fully in plain English — do not mix in any Hindi/Hinglish words.';
  }
  return '';
}

// Added 2026-09-23 (Ravi): live-tested and found that a generic "if the
// customer also asked something else, answer it first" instruction inside
// the Kundli/consultation flows' prompts was NOT reliably followed - a
// customer asking "free aur paid consultation mein kya farak hai?" in the
// middle of birth-detail collection got the question silently skipped in
// favour of the next slot question. Same lesson as enforcePriceSafety /
// enforceAddressCompleteness in replyEngine.js: a soft prose instruction
// isn't reliable enough on its own, so this is a deterministic detector +
// an explicit, high-priority per-turn instruction the flow files can inject
// only when the customer's message actually looks like a plan question.
const PLAN_QUESTION_PATTERN = /\b(farak|difference|antar|compare|comparison)\b|kya milta|kya milega|kya milta hai|what\s*(is|all is|'?s)?\s*included|whats included|paid.*worth|worth (it|paying)|which plan|kaunsa plan|plans? ke baare|consultation plans?\b.*\b(price|pricing|cost)\b/i;

function detectPlanQuestionNote(message) {
  if (PLAN_QUESTION_PATTERN.test(String(message || ''))) {
    return "IMPORTANT - PRIORITY: the customer's message this turn also asks about the paid consultation plans (pricing, what's included, or the difference from the free flow here). Using the CONSULTATION PLAN NOTES above, answer that clearly and specifically FIRST - this is not optional - before doing anything else this turn.";
  }
  return '';
}
...
module.exports = {
  ...
  detectReplyLanguageNote,
  detectPlanQuestionNote,
};
