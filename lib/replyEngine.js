...
// Defense-in-depth: model instructions are not sufficient to guarantee that a
// handbook/indicative price never reaches a customer. When live product data is
// absent, remove currency amounts from the generated customer-facing reply and
// replace the whole reply with a safe confirmation message if a price is detected.
function enforcePriceSafety(reply, liveProductData) {
  ...
  return 'Hari Om! Main aapko exact current price team se confirm karke batati hoon. Agar aap chahein, main aapke liye is Rudraksha ka suitable option bhi check karwa sakti hoon. - Mannat';
}
...
function normalizeResult(result, liveProductData) {
  const leadStatus = ['HOT', 'WARM', 'COLD', 'NOT_A_LEAD'].includes(result?.leadStatus)
    ...
    : 'WARM';
  return {
    reply: enforcePriceSafety(result?.reply, liveProductData),
    leadStatus,
