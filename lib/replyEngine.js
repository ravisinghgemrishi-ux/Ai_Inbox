...
// Keep the existing production model. Gemini documents gemini-3.6-flash as a stable model.
...
async function generateReply({ platform, type, message, contextText, liveProductData = '' }) {
  ...
  if (liveProductData) {
    userParts.push({
      text: `[CURRENT_LIVE_PRODUCT_DATA — may be used for current availability/price/product details: ${liveProductData}]`,
    });
  } else {
    userParts.push({ text: '[CURRENT_LIVE_PRODUCT_DATA: none available. Do not quote any current product price.]' });
  }
  ...
  if (!process.env.GEMINI_API_KEY) {
    return {
      ...
      productInterest: '',
      escalate: true,
      escalateReason: 'GEMINI_API_KEY is not configured.',
    };
  }
  ...
  for (let attempt = 0; attempt <= MAX_RETRIES; attempt += 1) {
    try {
      ...
      if (!res.ok) {
        ...
        // Retry only transient/rate-limit failures. Do not loop on bad auth or bad requests.
