...
async function notifyEscalation({ platform, contact, type, message, reply, reason, leadStatus, productInterest }) {
  ...
  try {
    ...
    // Do not block customer handling when Redis is unavailable.
  }
  ...
  if (!email.configured && !whatsapp.configured) {
    console.warn('[escalation] no alert webhook configured; escalation remains logged in lead sheet.');
  }
