...
const { BRAND_VOICE, detectReplyLanguageNote, looksHinglishOrHindi } = require('./knowledgeBase');
...
async function maybeHandleSellerInquiry({ enabled, type, memoryId, message, intent, existingMemory = '' }) {
  ...
  if (CANCEL_WORDS.test(message) && state) {
    ...
    return {
      reply: looksHinglishOrHindi(message)
        ? 'Bilkul, koi baat nahi! Jab bhi aap details share karna chahein, main yahin hoon. - Mannat'
        : "No problem at all! Whenever you'd like to share the details, I'm right here. - Mannat",
  ...
  try {
    ...
    return {
      reply: looksHinglishOrHindi(message)
        ? 'Isme thodi technical dikkat aa rahi hai - koi baat nahi, main aapki details humari sourcing team ko bhej rahi hoon, wo aapse jaldi contact karenge. - Mannat'
        : "There's a small technical hiccup on my end - no worries, I'm sending your details to our sourcing team, they'll contact you shortly. - Mannat",
...
async function handleCollecting(memoryId, state, message, languageNote = '') {
  ...
  if (missing.length > 0) {
    ...
    return {
      reply: reply || (looksHinglishOrHindi(message)
        ? `Dhanyavaad! Kya aap apna ${SLOT_LABELS[missing[0]]} bata sakte hain? - Mannat`
        : `Thank you! Could you share your ${SLOT_LABELS[missing[0]]}? - Mannat`),
  ...
  return {
    reply: reply || (looksHinglishOrHindi(message)
      ? `Dhanyavaad ${slots.name} ji! Humari sourcing team (Neel) aapko jald hi call karegi. - Mannat`
      : `Thank you, ${slots.name}! Our sourcing team (Neel) will call you back shortly. - Mannat`),
