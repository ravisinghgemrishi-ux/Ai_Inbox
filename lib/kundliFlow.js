...
const { buildProductCartLink } = require('./paymentLinks');
...
const SLOT_SCHEMA = {
  ...
  required: ['name', 'date', 'time', 'place', 'purpose'],
};
...
async function extractSlots(message, alreadyHave) {
  ...
  try {
    ...
    return { name: '', date: '', time: '', place: '', purpose: '' };
  }
}
...
const REQUIRED_SLOTS = ['name', 'date', 'time', 'place', 'purpose'];
...
async function maybeHandleKundliTurn({ enabled, type, memoryId, message, intent }) {
  ...
  if (!state && intent?.intent !== 'consultation') return null;
  ...
  if (!state) {
    state = { stage: 'collecting', slots: { name: '', date: '', time: '', place: '', purpose: '' }, createdAt: new Date().toISOString() };
  }
...
async function handleChoosingCategory(memoryId, state, message) {
  ...
  if (!wantsGemstone && !wantsRudraksha) {
    const reply = await generateStageReply({
      ...
      instruction: `Their message didn't clearly say gemstone or Rudraksha - ask again warmly, offering both options plainly.`,
    });
    await setSession(memoryId, state);
...
async function handleBuyDecision(memoryId, state, message) {
  ...
  if (state.category === 'rudraksha') {
    const variantId = state.chart?.shopify?.variantId || state.recommendation?.primary?.variantId;
    if (variantId) paymentLink = buildProductCartLink([{ variantId, quantity: 1 }]);
  }
  // Gemstones have no Shopify variant mapping yet (see gemstoneRecommender.js) -
  // paymentLink stays null and the reply below says the team will share it,
  // matching the existing price-safety pattern: never fabricate a link or price.
  ...
  const caseFile = [
    ...
    paymentLink ? `Payment link sent: ${paymentLink}` : 'Payment link: not available yet (team to share)',
  ].join(' | ');

  await clearSession(memoryId);

  const reply = await generateStageReply({
    contextSummary: `Customer confirmed they want to buy ${productName}. ${paymentLink ? `Payment link: ${paymentLink}` : 'No direct payment link is available for this item yet.'}`,
    instruction: paymentLink
      ? `Confirm warmly, share the payment link exactly as given, and let them know the team will also follow up to help finalize.`
      : `Confirm warmly and let them know the team will personally reach out shortly with the exact price and payment details, since this needs a quick manual check.`,
  });
  return {
    reply: reply || (paymentLink ? `Bahut accha! Yeh raha aapka payment link: ${paymentLink} - hamari team bhi aapse jaldi connect karegi. - Mannat` : `Bahut accha! Hamari team aapse jaldi contact karegi exact price aur payment details ke saath. - Mannat`),
    leadStatus: 'HOT', productInterest: productName || 'Kundli Consultation', escalate: true, escalateReason: `kundli_ready_to_buy | ${caseFile}`,
  };
}
